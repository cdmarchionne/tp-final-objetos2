...
//		inscribirAlumnoEnCatedra();
	...
	private static void inscribirAlumnoEnCatedra() {
		...
		PlanDeEstudio planDeEstudio= (PlanDeEstudio) buscar(PLAN_DE_ESTUDIO, Universidad.getInstance().getAreas().toArray());
		...
		if(alumnos.length > 0){
			...
			Materia analisis1 = (Materia) buscar(MATERIA_ANALISIS_I, Universidad.getInstance().getMaterias().toArray());
			Catedra catedraAnalisis1 = ((Catedra[]) analisis1.getCatedras().toArray())[0];
			
//			alumno.inscribirEnMateria(new InscripcionMateria(analisis1, catedraAnalisis1, alumno));
...
//		initPlanDeEstudio(carreraNueva,PLAN_DE_ESTUDIO);
	...
	private static void initPlanDeEstudio(Carrera carrera, String nombrePlan) {
		...
		Docente[] docentes = (Docente[]) Universidad.getInstance().getOficinaDeAlumnos().getDocentes().toArray();
	...
	private static void initMateriaYCatedra() {
		...
		Area areaMatematica = (Area) buscar(AREA_MATEMATICA, Universidad.getInstance().getAreas().toArray());
...
//		Carrera[] carreras = (Carrera[]) Universidad.getInstance().getCarreras().toArray();
//		
